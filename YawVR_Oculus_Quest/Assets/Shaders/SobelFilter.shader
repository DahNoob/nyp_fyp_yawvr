		...
		SubShader
		{
			...
			Pass
			{
				...
				half4 frag(Varyings input) : SV_Target
				{
					...
					float s = pow(1 - saturate(sobel(UnityStereoScreenSpaceUVAdjust(input.uv, _MainTex_ST))), 50);
	#ifdef RAW_OUTLINE
					return half4(s.xxx, 1);
	#else
					half4 col = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, UnityStereoScreenSpaceUVAdjust(input.uv, _MainTex_ST));
	#ifdef POSTERIZE
					col = pow(col, 0.4545);
