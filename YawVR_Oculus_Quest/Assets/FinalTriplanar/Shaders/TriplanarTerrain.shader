Shader "Custom/TriplanarTerrain"
{
	Properties
	{
		...
		sampler2D _MainTex, _MainTexSide, _NoiseTex;
		...
		struct Input
		{
			...
			void surf(Input IN, inout SurfaceOutputStandard o)
			{
				...
				float worldNormalDot = dot(o.Normal + (noiseTexture.y + (noiseTexture * 0.5)), IN.worldNormal.y);
				...
				float3 topTextureEdgeResult = (step(_TopSpread, worldNormalDot) * step(worldNormalDot, _TopSpread + _EdgeWidth)) * _EdgeColor;
